---
name: dataviz
description: Use this skill whenever you are about to create ANY chart, graph, plot, dashboard, or data visualization, in ANY output medium: an HTML or React artifact, inline SVG, plotting code in any library (matplotlib, plotly, d3, Recharts, and so on), or an image/PNG you will render and hand off. Read it BEFORE writing the first line of chart code, choosing chart colors, building a stat tile / meter / KPI row, or laying out a dashboard. Produces visualizations that read as one system, elegant, accessible, and consistent in light and dark, using a brand-neutral placeholder palette you swap for your own. Teaches a design-system-agnostic method: a form heuristic, a color formula with a runnable validator, mark specs, and interaction rules. A validated default palette is documented in `references/palette.md`; swap that file's values for your brand's. Triggers on: "chart", "graph", "plot", "data viz", "visualization", "dashboard", "analytics", "visualize data", "categorical colors", "sequential / diverging palette", "stat tile", "sparkline", "heatmap", "legend", "axis", "tooltip", "chart colors", "color by series".
---
...
This skill lives at `~/.agents/skills/dataviz`. Every `references/...` and
`scripts/...` path below is relative to that directory. If a relative path does not
resolve in your harness, use the absolute form:

```
~/.agents/skills/dataviz/scripts/validate_palette.js
~/.agents/skills/dataviz/scripts/validate_palette.py
```

Both validators are dependency-free and take identical arguments. Use the Python one
...
3. **VALIDATE the palette - run the script, don't reason about Delta E.**
   ...
   this skill's base directory), or `python scripts/validate_palette.py "<hex,hex,...>" --mode light`.
   In a browser, load it as `<script type="module">` in the chart's own page, where it reads
...
Then check the result against **`references/anti-patterns.md`**, the catalog
...
| `scripts/validate_palette.py` | Same validator, Python, for when `node` is unavailable |
